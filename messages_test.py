...
def instruction_message(count, current_user_id):
    ...
    string = "Неправильный формат данных! Я понимаю примитивные приветствия, команду 'stop' для отмены" + \
             "и формат: 'group_id: ....; period: ....' Подумай головой, пока не забанил тебя."
    vk_api2.messages.send(user_id=current_user_id, message=string, random_id=count)
    return count + 1
...
while run:
    ...
    if code == 3:
        if not have_a_task or current_user_id == master_id:
            ...
            next_time, count = first_process(count, master_id, group)
        else:
...
'''
            ...
            if t_finish > frequency_number * 60:
                vk_api2.messages.send(user_id=current_user_id, message="Can't work so fast((",
                                      random_id=count)
                count += 1
                next_time = datetime.datetime.now()
                frequency_number = 0
